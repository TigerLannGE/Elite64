...
export class TransactionsService {
  ...
  async createTransactionAndUpdateWallet(params: CreateTransactionParams) {
    ...
    if (params.amountCents === 0) {
      throw new BadRequestException(
        'Le montant de la transaction ne peut pas être zéro',
      );
    }
    ...
    if (!wallet) {
      throw new NotFoundException(
        `Portefeuille avec l'ID "${params.walletId}" introuvable`,
      );
    }
  ...
  async creditWallet(
    ...
    if (params.amountCents <= 0) {
      throw new BadRequestException(
        'Le montant de crédit doit être strictement positif',
      );
    }
  ...
  async debitWallet(
    ...
    if (params.amountCents <= 0) {
      throw new BadRequestException(
        'Le montant de débit doit être strictement positif',
      );
    }
  ...
  async findTransactionsByWallet(
    ...
    if (!wallet) {
      throw new NotFoundException(
        `Portefeuille avec l'ID "${walletId}" introuvable`,
      );
    }
